import { faAlignJustify, faPencil, faScissors, type IconDefinition } from "@fortawesome/free-solid-svg-icons";
...
function Canvas() {
  ...
  return (
    <div className="fixed top-0 right-8 exclude-tracking z-[5000]">
      <button
        className={`p-1 w-8 h-10 ${state.status === Status.inspecting ? 'bg-primary text-secondary' : 'bg-secondary text-primary'}`}
        onClick={() => {
...
const tools: ToolData[] = [
  ...
  },
  {
    ...
    shouldShow: ({ element }) => {
      ...
      return ['p', 'div', 'span'].includes(tagName) || tagName.startsWith('h') || tagName.startsWith('a');
    }
  },
];
...
function Toolbar({
  ...
  return (
    <div
      className={`fixed bg-secondary pointer-events-${pointerEvents} z-[5000]`}
      style={{
        top,
        right,
        width,
    ...
    >
      <div className="p-4">
          ...
          {tools.map((toolData, i) => {
            if (toolData.shouldShow({ element, location })) {
              return (
                <button
                  key={i}
                  className={`p-1 rounded ${isSelected && tool === toolData.name ? 'bg-primary text-secondary' : 'bg-secondary text-primary'}`}
        ...
        {tool && isSelected ? <ToolDisplay tool={tool} element={element} location={location}
